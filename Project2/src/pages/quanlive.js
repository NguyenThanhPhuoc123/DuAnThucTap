...
function createData(stt, booking, sove, status, ngaysd, ngayxv, cong) {
  return { stt, booking, sove, status, ngaysd, ngayxv, cong };
}
...
const rows = [
  ...
  createData(5, "Lasania Chiken Fri", 12345678, "Hết hạn", "", "19/1/2020", 1),
...
function Quanlive() {
  ...
  return (
    <div className='AppGlass1'>
      <h1>Danh sách vé</h1>
      <div>
        <input className='timkiem' id="search-input" placeholder='Tìm kiếm theo số vé' type="text" value={searchValue} onChange={handleSearchInputChange} />
        <button className='nutlocve' onClick={handleFilterClick}>Lọc vé</button>
        <button className='nutxuat' >Xuất file</button>
        <div className="quanlive-search">
          <label>
            <input
              type="checkbox"
              name="chuaSuDung"
            ...
            <TableBody style={{ color: "white" }}>
              {currentRows.map((row) => (
                ...
                >
                  ...
                  {/* stt, booking, sove, status,ngaysd,ngayxv,cong */}
